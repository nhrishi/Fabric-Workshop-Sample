...
func (t *AssetChaincode) issueAsset(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	assetAsBytes, err := stub.GetPrivateData("assetcollection", asset_key)
	if err != nil {
	...
	err = stub.PutPrivateData("assetcollection", asset_key, assetJSONasBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (t *AssetChaincode) readAsset(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var assetName, owner, jsonResp, asset_key string
	var err error
	...
	valAsbytes, err := stub.GetPrivateData("assetcollection", asset_key) //get the asset from chaincode state
	if err != nil {
...
func (t *AssetChaincode) transferAsset(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	var owner, assetName, newOwner, asset_key string
	if len(args) < 3 {
		return shim.Error("Incorrect number of arguments. Expecting 3")
	}
	...
	err = stub.PutPrivateData("assetcollection", asset_key, assetJSONasBytes) //write the asset
	if err != nil {
		return shim.Error(err.Error())
	}
